...
#define _XOPEN_SOURCE 700 //para puder usar o strptime no windows para que a stdio se comporte de maneira diferente.
#define _GNU_SOURCE
#define __USE_XOPEN
#include <iostream>
#include<stdio.h>
...
void flush_in(){ // codigo para limpeza de buffer do teclado..
 ...
 void junta_valorManual(point * lstAux){ //inserir no fim da fila questao
   ...
   if((v=listavaziamaquinas(*lstAux))==0){
     ...
     puts("Insira a data no formato dd/mm/aaaa hh:mm "); scanf("%s", aux);
     strptime(aux, "%d-%m-%Y %H:%M", &novo->data);
     mktime(&novo->data);
...
void saveDataBase(maquinas vetor[TAMANHO], int counter){
  ...
  if(saveBin==NULL){
    ...
    while(i<=counter){
      ...
      while(atual != NULL){
        fwrite(atual, sizeof(dados), lista, saveBin);
        atual=atual->prox;
      }
      i++;
    }
...
int loadDataBase(maquinas vetor[TAMANHO], int counter){ //subprograma que vai fazer a leitura do que está no ficheiro..
  ...
  if(loadBin==NULL){
    ...
    while(i<=counter){
      ...
      atual=vetor[i].head;
      while(atual != NULL){
        fread(atual, sizeof(dados), 1, loadBin);
        atual=atual->prox;
      }
    }
