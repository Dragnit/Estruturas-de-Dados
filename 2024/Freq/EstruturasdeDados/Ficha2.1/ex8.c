...
#include<ctype.h> //para verificar se é isalpha, se são letras o que se está a escrever na parte do nome de aluno.
 ...
 Programa/Exercicio: Exercicio 8 / Ficha 2
 --------------------------------------------------------------
 --------------------------------------------------------------
 */
   ...
   int RemoveStudent(int counter, char memoryStudent[50][50], int memoryNumber[50]){
     ...
     void saveData(char memoryStudent[50][50], int memoryNumber[50]){
       ...
       saveStudent = fopen("SaveAlunos.dat", "wb"); //gravação de alunos associados
       ...
       if(saveStudent==NULL){
         ...
         fwrite(memoryStudent, 50* sizeof(char[50]), 1, saveStudent);
         }
       fclose(saveStudent);


     saveNumber = fopen("SaveNumeros.dat", "wb"); //gravação de numeros de alunos
     ...
     if(saveNumber==NULL){
       ...
       fwrite(memoryNumber, sizeof(int[50]), 1, saveNumber);
     }
     fclose(saveNumber);
   ...
   int loadData(int valor, int memoryNumber[50], char memoryStudent[50][50]){
     ...
     readStudent = fopen("SaveAlunos.dat", "rb");          //ler alunos associados

     if(readStudent==NULL){
       printf("Nao ha dados sobre alunos anteriormente guardados\n");
       readStudent = fopen("SaveAlunos.dat", "wb"); //caso seja a primeira vez a iniciar o programa ele cria um documento para guardar..
     }
     else{
       fread(memoryStudent, 50* sizeof(char[50]), 1, readStudent);
       }
     fclose(readStudent);

     readNumber = fopen("SaveNumeros.dat", "rb");      //ler numeros de alunos

     if(readNumber==NULL){
       printf("Nao foi possivel encontrar dados sobre Numeros\n");
       readNumber = fopen("SaveNumeros.dat", "wb");
     }
     else{
       fread(memoryNumber, sizeof(int[50]), 1, readNumber);
       }

     fclose(readNumber);

     for(int a=0; memoryNumber[a]!=-1; ++a){
       valor++;
     }
