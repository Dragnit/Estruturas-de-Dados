...
int loadDataBase(maquinas vetor[TAMANHO], int counter){ //subprograma que vai fazer a leitura do que está no ficheiro..
  ...
  if(loadBin==NULL){
    ...
    while(i<counter){
      fread(&vetor[i], sizeof(maquinas), 1, loadBin);
      ...
      if(lista>0){ //para por no inicio da lista
        ...
        fread(novo, sizeof(dados), 1, loadBin);
        no->prox = NULL;
        vetor[i].head = novo;
        atual=vetor[i].head;
        atual = atual->prox; //basicamente fica NULL.
      }
      lista=(lista-1);
      while(lista != 0){
