 ...
 */
 ...
 int inserirDados( funcionario * dados){
   int auxNumero;
   float salar;
   funcionario * pontAux, *criaAux;
   printf("Insira numero de funcionario: ");
   if(scanf("%d", &auxNumero)){
     if(auxNumero==dados->numero){ //comparacao na primeira estrutura.
       printf("\nNumero de funcionario já existe\n");
       return 0;
     }
     pontAux=dados->proximo; //pontAux toma o valor do ponteiro inicial(que aponta para a primeira estrutura) dados que aponta para o proxima estrutura
     if(dados->proximo==NULL){
       printf("Numero inserido com sucesso\n");
       dados->numero=auxNumero; // dou o valor ao devido local na estrutura.
       flush_in();
       printf("Insira o nome do funcionario: "); fgets(dados->nome, 58, stdin); // em vez de usar o strcpy inseri directamente na estrtura.
       auxNumero=(strlen(dados->nome)-1); // ainda pensei em usar o scanf[] mas devido que uso mais o fgets para ficheiros decidi começar a treinar mais este.
       dados->nome[auxNumero]='\0'; // basicamente o \n estava a dar problemas depois para listar, decidi eliminar logo desde inicio
       printf("Funcionario inserido com sucesso\n");

       printf("Insira a tarefa do funcionario: "); fgets(dados->tarefa, 48, stdin); // inserir tarefas do funcionario
       auxNumero=(strlen(dados->tarefa)-1);
       dados->tarefa[auxNumero]='\0'; // basicamente o \n estava a dar problemas depois para listar, decidi eliminar logo desde inicio
       ...
       criaAux = (funcionario*)malloc(sizeof(funcionario)); //assim vai alocando a memoria a cada vez que é inserido um funcionario.
       criaAux->proximo=NULL; //deixar a proxima estrutura com o ponteiro a NULL
       pontAux->proximo = criaAux;
       return 1;
     }

     while(pontAux != NULL){ //A PARTIR DAQUI É QUANDO JÁ EXISTE MAIS QUE UM FUNCIONARIO AVISO!!
       ...
       pontAux=pontAux->proximo; //dentro da estrutura onde esta ele aponta para a proxima estrutura.
     }
     printf("Numero inserido com sucesso\n");
   ...
   criaAux = (funcionario*)malloc(sizeof(funcionario));
   criaAux->proximo=NULL; //deixar a proxima estrutura com o ponteiro a NULL
   pontAux->proximo = criaAux; //assim vai alocando a memoria a cada vez que é inserido um funcionario.
   return 1;
 }
...
*/
 int main(){
   int opcao=0, bin;
   char auxNome[60];
   funcionario *dados, *ponteiro; //fica apontar para o inicio..
   ponteiro = (funcionario*)malloc(sizeof(funcionario)); //vetor linear de tipo funcionario / alocar tamanho vezes tipos de espaço de dimensao funcionario.
   ...
   do{
     ...
     if(isdigit(opcao)==0){ // para saber se é número uso a função isdigit que retorna 0 caso o caracter não seja numerico
       switch (opcao) {
         case 1: bin=inserirDados(dados); break;
