...
void flush_in(){ // codigo para limpeza de buffer do teclado..
 ...
 void junta_no_ini_listaManual(point * lst) { //INSERÇÃO MANUAL DOS DADOS
   ...
   if (no == NULL){
    ...
    no->data->tm_sec = seg;
    /*
    no->data->tm_wday = 0; deixo como exemplo isto em comentario para se perceber o mktime..
    puts("aqui");
    no->data->tm_yday = 0;
    puts("aqui2");
    no->data->tm_isdst = -1;
 ...
 void junta_valorManual(point * lstAux){ //inserir no fim da fila questao
   ...
   if((v=listavaziamaquinas(*lstAux))==0){
     ...
     /*
     novo->data->tm_wday = 0; deixei aqui para testes futuros de manipulação e deixo como exemplo para se saber o que o mktime está a fazer dentro da funcao..
     novo->data->tm_yday = 0;
     novo->data->tm_isdst = -1;
     */
     val=mktime(novo->data);
     if( val == -1 ) {
      printf("Error: unable to make time using mktime\n");
      return;
     }
...
void saveDataBase(maquinas vetor[TAMANHO], int counter){
  ...
  if(saveBin==NULL){
    ...
    while(i<counter){
      ...
      while(atual != NULL){
        fwrite(&(atual->data->tm_sec), sizeof(int), 1, saveBin);
        fwrite(&(atual->data->tm_min), sizeof(int), 1, saveBin);
        fwrite(&(atual->data->tm_hour), sizeof(int), 1, saveBin);
        fwrite(&(atual->data->tm_mday), sizeof(int), 1, saveBin);
        fwrite(&(atual->data->tm_mon), sizeof(int), 1, saveBin);
        fwrite(&(atual->data->tm_year), sizeof(int), 1, saveBin);
        fwrite(&(atual->data->tm_wday), sizeof(int), 1, saveBin);
        fwrite(&(atual->data->tm_yday), sizeof(int), 1, saveBin);
        fwrite(&(atual->data->tm_isdst), sizeof(int), 1, saveBin);
...
int loadDataBase(maquinas vetor[TAMANHO], int counter){ //subprograma que vai fazer a leitura do que está no ficheiro..
  ...
  if(loadBin==NULL){
    ...
    while(i<counter){
      ...
      vetor[i].head=crialistamaquinas();
      atual=vetor[i].head; //o mesmo que estar a null
      if(lista>0){ //para por no inicio da lista
        puts("Lista");
        no = (point)malloc(sizeof(dados));
        no->data = localtime(&dat);
        fread(&(no->data->tm_sec), sizeof(int), 1, loadBin);
        fread(&(no->data->tm_min), sizeof(int), 1, loadBin);
        fread(&(no->data->tm_hour), sizeof(int), 1, loadBin);
        fread(&(no->data->tm_mday), sizeof(int), 1, loadBin);
        fread(&(no->data->tm_mon), sizeof(int), 1, loadBin);
        fread(&(no->data->tm_year), sizeof(int), 1, loadBin);
        fread(&(no->data->tm_wday), sizeof(int), 1, loadBin);
        fread(&(no->data->tm_yday), sizeof(int), 1, loadBin);
        fread(&(no->data->tm_isdst), sizeof(int), 1, loadBin);
      ...
      while(lista != 0){
        ...
        novo->data = localtime(&dat);
        fread(&(novo->data->tm_sec), sizeof(int), 1, loadBin);
        fread(&(novo->data->tm_min), sizeof(int), 1, loadBin);
        fread(&(novo->data->tm_hour), sizeof(int), 1, loadBin);
        fread(&(novo->data->tm_mday), sizeof(int), 1, loadBin);
        fread(&(novo->data->tm_mon), sizeof(int), 1, loadBin);
        fread(&(novo->data->tm_year), sizeof(int), 1, loadBin);
        fread(&(novo->data->tm_wday), sizeof(int), 1, loadBin);
        fread(&(novo->data->tm_yday), sizeof(int), 1, loadBin);
        fread(&(novo->data->tm_isdst), sizeof(int), 1, loadBin);
